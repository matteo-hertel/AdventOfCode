...
debug = 0;
const mapping = {
    "inc": function sum(a, b) { return a + b },
    "dec": function sub(a, b) { return a - b },
    "==": function equal(a, b) { return a == b },
    "===": function strictEqual(a, b) { return a === b },
    "!=": function notEqual(a, b) { return a != b },
    "!==": function notStrictEqual(a, b) { return a !== b },
    ">": function greaterThan(a, b) { return a > b },
    "<": function lessThen(a, b) { return a < b },
    ">=": function greaterThanOrEqual(a, b) { return a >= b },
    "<=": function lessThanOrEqual(a, b) { return a <= b },
}
function processComparisonOperator(operator, a, b) {
    if (typeof mapping[operator] !== "function") {
        throw new Error("Invaid operator")
    }
    return mapping[operator](a, b);
}

function getPosition(i) {
    return parseInt(i, 36) - 10//this is quite neat :D
}
...
function processInstructions(acc, i) {
    acc.registry[i.registryPosition] = acc.registry[i.registryPosition] || 0;
    acc.registry[i.a] = acc.registry[i.a] || 0;
    if (mapping[i.operator](acc.registry[i.a], i.b)) {
        acc.registry[i.registryPosition] = i.operation(acc.registry[i.registryPosition], i.value)
    }
    let potentialBiggest = acc.registry.reduce(getBiggest)
    if (potentialBiggest > acc.biggest) {
        acc.biggest = potentialBiggest
    }
    return acc;
}

function parseInstructions(i) {
    let [registryPosition, operation, value, nah, a, operator, b] = i.split(" ");
    return {
        registryPosition: getPosition(registryPosition),
...
function processData(input, registry) {
    let processedData = input
        .trim()
        .split("\n")
        .map(parseInstructions)
        .reduce(processInstructions, { biggest: 0, registry });

    return {
        result: processedData.registry.reduce(getBiggest),
        biggest: processedData.biggest
    }
}
...
`;
console.log(processData(debug ? testInput : input, []))
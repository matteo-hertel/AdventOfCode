...
*/
//First problem
function getBaseProgram(input) {
    return input
        .trim()
        .split('\n')
        .map(getKey)
        .filter(getBaseKey.bind(null, input))
        .join();
}
//second problem
function getUnblanacedProgram(input) {
    return getTree(input)
        .children
        .filter(filterIsBalanced)
        .reduce(reducerGetCorrectWeight, 0);
}
//utils functions
function getBaseKey(input, i) {
    return input.match(new RegExp(`(${i}\\b)`, "g")).length === 1;
}
...
function parseData(acc, i) {
    let [key, weight] = i.split(" ");
    weight = parseInt(weight.match(/\d+/g).join());
    acc[key] = {
        isBalanced: true,
        balance: weight,
        key,
    ...
    if (i.indexOf("->") !== -1) {
        i.slice(0, i.indexOf("->"))
        acc[key].children = i.slice(i.indexOf("->") + 3).split(", ");
    }
    return acc;
}

function getTree(input) {
    return buildTree(normaliseData(input), getBaseProgram(input))
}

function buildTree(tree, key) {
    const node = tree[key];
    if (node.children.length) {
        node.children = node.children.map(buildTree.bind(null, tree));
        node.balance = node.weight + node.children.reduce(sumWeight, 0);
        node.isBalanced = node.children.every((val, i, arr) => val.balance === arr[0].balance);
    }
    return node;
}
function reducerGetCorrectWeight(acc, i) {
    const correctBalance = i.children[0].balance

    function getDifference(acc, i) {
        return i.weight - (i.balance - correctBalance);
    }

    return i.children
        .filter(filterOutCorrectValues)
        .reduce(getDifference, 0);
};
...
function sumWeight(a, i) {
    return a + i.balance;
};

function filterOutCorrectValues(val, i, arr) {
    return val.balance !== arr[0].balance
}

function filterIsBalanced(i) {
    return !i.isBalanced;
}
...
`;
console.log(getBaseProgram(codeInput));
console.log(getUnblanacedProgram(codeInput));

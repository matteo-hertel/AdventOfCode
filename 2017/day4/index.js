...
function splitBy(separator) {
  return function splitBySeparator(arr) {
    return arr.split(separator);
  };
}

function isDuplicated(item, index, arr) {
  return arr.indexOf(item) !== index;
}

function filterMultidimesionalDuplictes(arr) {
  return arr.filter(isDuplicated);
}
...
function countValid(acc, errors) {
  if (errors.length) {
    return acc;
  }
  return (acc += 1);
 ...
 * I LOVE this one!
 */
function applyToChildren(type, fn) {
  return function(arr) {
    return arr[type](fn);
  };
}

function countValidPassphrases1(passphrases) {
  return passphrases
    .trim()
    .split("\n")
    .map(splitBy(" "))
    .map(applyToChildren("filter", isDuplicated))
    .reduce(countValid, 0);
}

function countValidPassphrases2(passphrases) {
  return passphrases
    .trim()
    .split("\n")
    .map(splitBy(" "))
    .map(applyToChildren("map", sortString))
    .map(applyToChildren("filter", isDuplicated))
    .reduce(countValid, 0);
}
...
console.log({
  first: countValidPassphrases1(passphrase), //383
  second: countValidPassphrases2(passphrase) // 265
}); 
